...
/*
export const toFilters = (filters?: any[]): string => {
...
export const toFilters = (properties: Property[], version: string, filters?: { [id: string]: any[]; }): string => {
    if (!filters) {
        return ''
    }
    ...
    const filter = keys.map(u => {
        const property = properties.find(r => r.name === u)

        return filters[u].map(r => {
            return `${u} eq ${toQueryId(property!, r, version)}`
        })
            .join(' or ')
    })
        .map(t => `(${t})`)
...
export const toAscDesc = (order: OFluiOrder) => {
    return order === OFluiOrder.ascending
        ? 'asc'
        : 'desc'
}
    ...
    navigationProps.forEach(h => {
        if (item[h.name]) {
            resultItem[h.name] = h.isCollection
                ? item[h.name].map((h: OFluiLookup) => h.sourceItem)
                : item[h.name].sourceItem
        }
    })
...
export const toUrlQuery = (view: OFluiView, version: string, properties: Property[], key: string, skip?: number) => {
    const selectProperties = view.query.fields.filter(f => properties.find(a => a.name === f && a.type !== PropertyType.navigation))

    if (view.query.fields.indexOf(key) === -1) { selectProperties.push(key) }
    ...
    const filters = toFilters(properties, version, view.query.filters)

    return `${select}&${expand}&${order}&${pager}&${filters}`

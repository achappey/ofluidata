...
export const useListFilter = (currentFilters?: { [id: string]: any[]; },
    ...
    const filterToggle = getFilterOptions ? {
        ...filterPaneButton,
        buttonStyles: showFilterPane ? { root: { backgroundColor: DefaultPalette.neutralLighter } } : undefined,
        onClick: () => setShowFilterPane(!showFilterPane),
    } : undefined;

    const applyFilters = onFiltersChanged ? (filters: any[]) => {
    ...
    const onFilterCleared = getFilterOptions && onFiltersChanged ? (column: OFluiColumn) => {
        const filter = {
            ...currentFilters
        }
    ...
    const getOptions = filterPanelProperty && getFilterOptions ?
        ...
        //      options: h,
        //      selected: []
        /*   selected: currentView.query.filters
               && currentView.query
                   .filters[filterPanelProperty.name] ?
               currentView.query
                   .filters[filterPanelProperty.name]
               : []*/
        //    }
